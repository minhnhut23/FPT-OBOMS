...
public class ShopDAO
{
    ...
    public async Task<List<GetTotalShopStatisticResponseDTO>> GetDailyRevenue(GetTotalShopStatisticByMonthRequestDTO request, string token)
    {
        var handler = new JwtSecurityTokenHandler();
        var jwtToken = handler.ReadJwtToken(token);
        var claims = jwtToken.Claims.ToDictionary(c => c.Type, c => c.Value);
        ...
        var shops = await _client
            .From<Shop>()
            .Select("id")
            .Where(x => x.OwnerId == profile.Id)
            .Get();

        var shopIds = shops.Models.Select(s => s.Id).ToList();
        ...
        var bills = await _client
            ...
            .Where(x => shopIds.Contains(x.ShopId) && x.CreatedAt >= startDate && x.CreatedAt <= endDate)
            .Get();

        var groupedRevenue = bills.Models
        .GroupBy(b => b.CreatedAt.Date)
        .Select(g => new GetTotalShopStatisticResponseDTO
        {
            Date = g.Key,
